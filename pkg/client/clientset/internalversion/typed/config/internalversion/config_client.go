...
// ConfigClient is used to interact with features provided by the config.morfic.io group.
...
// NewForConfigAndClient creates a new ConfigClient for the given config and http client.
// Note the http client provided takes precedence over the configured transport values.
func NewForConfigAndClient(c *rest.Config, h *http.Client) (*ConfigClient, error) {
	config := *c
	if err := setConfigDefaults(&config); err != nil {
		return nil, err
	}
...
func setConfigDefaults(config *rest.Config) error {
	...
	if config.GroupVersion == nil || config.GroupVersion.Group != scheme.Scheme.PrioritizedVersionsForGroup("config.morfic.io")[0].Group {
		gv := scheme.Scheme.PrioritizedVersionsForGroup("config.morfic.io")[0]
